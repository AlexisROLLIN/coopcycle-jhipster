...
describe('Order e2e test', () => {
  ...
  /* it('should create and save Orders', async () => {
        ...
        await promise.all([
            orderUpdatePage.orderStateSelectLastOption(),
            orderUpdatePage.setDeliveryTimeInput('01/01/2001' + protractor.Key.TAB + '02:30AM'),
            orderUpdatePage.restaurantSelectLastOption(),
            orderUpdatePage.userSelectLastOption(),
            orderUpdatePage.paymentSelectLastOption(),
        ]);

        expect(await orderUpdatePage.getDeliveryTimeInput()).to.contain('2001-01-01T02:30', 'Expected deliveryTime value to be equals to 2000-12-31');
  ...
  /* it('should delete last Order', async () => {
        ...
        expect(await orderDeleteDialog.getDialogTitle())
            .to.eq('coopcycleApp.order.delete.question');
